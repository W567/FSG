...
- **`pdfs/`** — Manuscripts related to this project
- **`scripts/`**
...
Scripts of AutoWS in [FSG/scripts/autows](scripts/autows) is used to generate robot fingertip workspace clouds based on robot urdf models. 
...
We provide scripts to sample point cloud from object .obj mesh file and extract desired partial point cloud. Corresponding scripts are stored in [FSG/assets/obj/toolkits](assets/obj/toolkits).
...
```
Generated workspace clouds will be saved under [FSG/workspace/](workspace/), with robot name as the folder name.
...
Generated workspace clouds will be saved under [FSG/workspace/](workspace/), with object name as the folder name (currently only for srh).
...
please add the new combination to [FSG/src/main.cc](src/main.cc),
add corresponding point cloud type (related to $N_a$ only) to [FSG/include/fsg/cloud_type.h](include/fsg/cloud_type.h) if necessary, and rebuild the project.

To generate the workspace clouds of the new robot hand, please add the autows.yaml configuration file to [FSG/config](config/)/*robot_hand_name*,
